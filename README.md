...
- `never` — never inject the workpad into LLM context; the LLM accesses workpad state only via its tools (`mini-self-org-workpad` to update, `mini-self-org-workpad-get` to read, and `mini-self-org-history` to recall past snapshots).
